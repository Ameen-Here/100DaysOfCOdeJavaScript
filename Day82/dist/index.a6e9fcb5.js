...
})({"4mRvh":[function(require,module,exports) {
"use strict";
var HMR_HOST = null;
var HMR_PORT = null;
var HMR_SECURE = false;
var HMR_ENV_HASH = "d6ea1d42532a7575";
module.bundle.HMR_BUNDLE_ID = "9891279ca6e9fcb5";
...
},{}],"grH6m":[function(require,module,exports) {
...
const checkLimit = (spendingLimits1, user)=>spendingLimits1?.[user] ?? 0
;
// Pure Function :D
const addExpense = function(state, spendingLimit, value, description, user = "jonas") {
...
const checkBudget = (finalBudget1, spendingLimit)=>finalBudget1.map((budget1)=>budget1.value < -checkLimit(spendingLimit, budget1.user) ? {
            ...budget1,
            flag: "limit"
        } : budget1
    )
;
const logBigExpenses = function(finalBudget2, limit) {
    //   let output = "";
    //   for (var entry of budget)
    //     output += entry.value <= -limit ? `${entry.description.slice(-2)} / ` : "";
    //   output = output.slice(0, -2); // Remove last '/ '
    //   console.log(output);
    return finalBudget2.filter((budget2)=>budget2.value <= -limit
    )// .map((expense) => expense.description.slice(-2))
    // .join("/");
    .reduce((accu, expense)=>`${accu} ${expense.description.slice(-2)}/`
    , "").slice(0, -1);
};
// Tried composing (Expirimental)
const finalBudget = addExpense(addExpense(addExpense(budget, spendingLimits, 10, "Pizza 🍕"), 100, "Going to movies 🍿", "Matilda"), spendingLimits, 200, "Stuff", "jonas");
...
},{}]},["4mRvh","grH6m"], "grH6m", "parcelRequire4878")

//# sourceMappingURL=index.a6e9fcb5.js.map
