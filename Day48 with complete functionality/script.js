...
const account1 = {
  ...
  movements: [
    ...
    [79.97, "2022-01-30T23:36:17.929Z"],
    [1300, "2022-02-01T10:51:36.790Z"],
  ],
  interestRate: 1.2, // %
  pin: 1111,
...
const formattingDate = function (currentDate, cond = true, locale = "en-GB") {
...
// Number printing according to locale

const numberConverter = function (num, locale) {
  const options = {
    style: "currency",
...
const startResetTimer = function () {
...
const logIn = function () {
  ...
  labelDate.textContent = formattingDate(
    currentDate,
    false,
...
function calcPrintBalance(acc) {
  ...
  labelSumOut.textContent = `${numberConverter(
    Math.abs(outflow),
    currentAccount.locale
  )} `;

  labelSumIn.textContent = `${numberConverter(income, currentAccount.locale)} `;

  labelSumInterest.textContent = `${numberConverter(
    interest,
    currentAccount.locale
  )}`;

  labelBalance.textContent = `${numberConverter(
    acc.balance,
    currentAccount.locale
  )} `;
}
...
const movementFun = function (accountt, sort = false) {
  ...
  move.forEach(function (mov, i) {
    ...
    const dateMove = new Date(`${mov[1]}`);
    const displaydate = formattingDate(dateMove, true, currentAccount.locale);
    ...
    <div class="movements__row">
          ...
          <div class="movements__value">
            ${numberConverter(mov[0], currentAccount.locale)}
          </div>
        </div>
    `;
